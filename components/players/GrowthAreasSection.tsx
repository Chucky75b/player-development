...
import { saveGrowthArea, deleteGrowthArea } from "@/app/players/[id]/actions";
...
function EditForm({
  ...
  playerId: string;
  item: GrowthArea | null;
  onSaved: (item: GrowthArea) => void;
  onCancel: () => void;
}) {
  const [title, setTitle] = useState(item?.title ?? "");
  const [description, setDescription] = useState(item?.description ?? "");
  const [category, setCategory] = useState(item?.category ?? "technical");
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      const saved = await saveGrowthArea(playerId, {
        id: item?.id,
        title,
        description,
        category,
...
export function GrowthAreasSection({ playerId, initialItems }: Props) {
  const [items, setItems] = useState(initialItems);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [adding, setAdding] = useState(false);
  const [, startTransition] = useTransition();

  const handleSaved = (saved: GrowthArea) => {
    setItems((prev) => {
  ...
  const handleDelete = (id: string) => {
    ...
    startTransition(async () => {
      await deleteGrowthArea(playerId, id);
    });
  };
