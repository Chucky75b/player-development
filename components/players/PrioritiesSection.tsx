...
import { savePriority, deletePriority } from "@/app/players/[id]/actions";
...
function EditForm({
  ...
  const handleSave = () => {
    ...
    startTransition(async () => {
      const saved = await savePriority(playerId, {
        id: item?.id,
        title,
        description,
        status,
...
export function PrioritiesSection({ playerId, initialItems }: Props) {
  const [items, setItems] = useState(initialItems);
  const [editingId, setEditingId] = useState<string | null>(null);
  const [adding, setAdding] = useState(false);
  const [, startTransition] = useTransition();

  const handleSaved = (saved: Priority) => {
    setItems((prev) => {
  ...
  const handleDelete = (id: string) => {
    ...
    startTransition(async () => {
      await deletePriority(playerId, id);
    });
  };
